 ...
 *  - `.sankofa.json` — can hold a project-scoped Deploy Token after
 *    `sankofa login --project`. Leaks auth if committed.
 *  - `build/` — the CLI's `--output-dir` (OTA archive, native preview artifact,
 *    signed binary, xcodebuild logs — all under `build/`, so one line covers it).
 *  - `.sankofa/build/`, `.sankofa/baseline/` — transient, regenerated each
 *    release/patch.
 * NOT ignored (intentionally committed): `.sankofa/dynamic_modules/` and
 * `.sankofa/flutter-version`, so fresh clones + CI build without re-running init.
 */
const GITIGNORE_ENTRIES: Array<{ pattern: string; comment?: string }> = [
  { pattern: '.sankofa.json', comment: 'Sankofa — CLI credentials / deploy token (never commit)' },
  { pattern: 'build/', comment: "Sankofa — build + OTA output (the CLI's --output-dir)" },
  {
    pattern: '.sankofa/build/',
    comment:
      'Sankofa — transient patch/release build output (regenerated each run).\n' +
      '# The rest of .sankofa/ (dynamic_modules/, flutter-version) IS committed so a\n' +
      '# fresh clone + CI run `flutter pub get` without re-running `sankofa init`.',
  },
  { pattern: '.sankofa/baseline/' },
];
...
async function installDeployFlutter(project: ProjectInfo, endpoint: string, chalk: any) {
  ...
  //    so the customer's pubspec doesn't carry a GitHub URL. It's committed
  //    (only the transient .sankofa/build/ + baseline/ are git-ignored), so a
  //    fresh clone / CI gets a working `flutter pub get` with no re-init.
 ...
 * `<project>/.sankofa/dynamic_modules/` (committed — only transient
 * `.sankofa/` subdirs are git-ignored, by the managed block). Idempotent —
 * re-running is a `git fetch + reset` to keep the local copy current.
...
async function ensureVendoredDynamicModules(projectRoot: string, chalk: any): Promise<boolean> {
  ...
  // NOTE: we deliberately do NOT git-ignore .sankofa/dynamic_modules/ or
  // .sankofa/flutter-version. Committing them lets a fresh clone (and CI) run
  // `flutter pub get` without re-running `sankofa init` — the pubspec's
  // `dependency_overrides: dynamic_modules: path: .sankofa/dynamic_modules`
  // resolves immediately. Only the transient .sankofa/build/ + .sankofa/baseline/
  // are ignored, via the single managed block in ensureSankofaGitignore().
  return true;
}
