 ...
 */
export function detectAppVersion(platform) {
    ...
    if (platform === 'ios') {
        try {
            const plistOutput = execSync(`find ios -name Info.plist -not -path "*/Pods/*" -not -path "*/build/*" | head -1`, { encoding: 'utf-8' }).trim();
            if (plistOutput) {
                const version = execSync(`/usr/libexec/PlistBuddy -c "Print :CFBundleShortVersionString" "${plistOutput}"`, { encoding: 'utf-8' }).trim();
                if (version)
                    return version;
            }
        }
 ...
 */
export function bundleJS(platform, entryFile, outputPath) {
    const isExpo = existsSync(join(process.cwd(), 'node_modules', 'expo'));
    if (isExpo) {
        // Expo: use npx expo export:embed
        execSync(`npx expo export:embed --platform ${platform} --entry-file ${entryFile} --bundle-output ${outputPath} --dev false`, { stdio: 'inherit' });
    }
    else {
        // Bare RN: use react-native bundle
        execSync(`npx react-native bundle --platform ${platform} --entry-file ${entryFile} --bundle-output ${outputPath} --dev false`, { stdio: 'inherit' });
    }
}
/**
 ...
 */
export function buildNative(platform, outputDir, outputFormat) {
    const isExpo = existsSync(join(process.cwd(), 'node_modules', 'expo'));
    if (platform === 'ios') {
        if (isExpo) {
            ...
            execSync(`cd ios && xcodebuild -workspace *.xcworkspace -scheme * -configuration Release -archivePath ${outputDir}/app.xcarchive archive`, { stdio: 'inherit' });
        }
        ...
        const defaultPath = join(process.cwd(), 'android', 'app', 'build', 'outputs', outputFormat === 'apk' ? 'apk' : 'bundle', 'release', `app-release.${ext}`);
        return defaultPath;
    }
}
