...
import { detectAppVersion, bundleJS, buildNative, computeSHA256, formatBytes, getFileSize, } from '../utils/bundler.js';
...
export const releaseCommand = new Command('release')
    ...
    try {
        ...
        const existing = releases.find((r) => r.target_binary_version === appVersion && r.platform === platform);
        if (existing) {
            checkSpinner.fail(`Version ${chalk.bold(appVersion)} already has a release (${chalk.dim(existing.label)}). Use ${chalk.cyan('sankofa patch ' + platform)} instead.`);
            process.exit(1);
        }
