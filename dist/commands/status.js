...
export const statusCommand = new Command('status')
    ...
    try {
        ...
        for (const r of releases) {
            ...
            console.log(`  ${chalk.bold(r.label.padEnd(24))} ${r.platform.padEnd(10)} ${r.target_binary_version.padEnd(10)} ${status.padEnd(30)} ${chalk.dim(`${installs} installs`)}${rollbacks > 0 ? chalk.red(` ${rollbacks} rollbacks`) : ''}`);
        }
        console.log('');
    }
    catch (err) {
